...
async function fetchPlayerHtml(pageUrl) {
  const r = await fetch(pageUrl, { headers: { "User-Agent": UA, "Accept-Language": "ar,en;q=0.9" } });
  ...
  const vp = await fetch(SITE + "/video_player?player_token=" + token, {
    headers: { "User-Agent": UA, "Referer": pageUrl, "Accept-Language": "ar,en;q=0.9" },
  });
...
module.exports = { extractFromPage, extractM3u8, fetchPlayerHtml };
